...
const Eigen::IOFormat EIGEN_HEAVY_FORMAT = {Eigen::FullPrecision, 0, ", ", ";\n", "[", "]", "[", "]"};
...
template <typename... T>
void print(const base::PrintMode& mode, const std::string& node_name, const std::string& content, T&&... args) {
  fmt::print(
      fmt::fg(base::PRINT_COLOR.at(mode)),
      base::PRINT_PREFIX.at(mode) + " " + (node_name == "" ? "" : "@" + node_name + ": ") + content + '\n',
      args...
  );
}
