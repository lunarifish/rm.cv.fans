...
aimer::AimInfo Sentry::get_aim() const {
  return this->armor_model.get_aim(/*passive=*/
                                   ::base::get_param<bool>("auto-aim.enemy-model.old-sentry.passive-mode.enabled")
  );
}
...
void Infantry::draw_aim(cv::Mat& img) const {
  ...
  // }
  this->lmtd_top_model.draw_armors(img, this->converter->get_img_t(), this->converter, this->state);
}

/** @class BalanceInfantry */

BalanceInfantry::BalanceInfantry(aimer::CoordConverter* const converter, aimer::EnemyState* const state)
    : converter(converter),
      state(state),
      // C++ 的规则真是一团糟！
      armor_model{
          aimer::ArmorModel(converter, state, aimer::MODEL_ARMOR_CREDIT_TIME.at(aimer::ModelType::BALANCE_INFANTRY))},
...
const std::unordered_map<
    ...
    std::function<std::unique_ptr<
        aimer::EnemyModelInterface>(aimer::CoordConverter* const converter, aimer::EnemyState* const state)>>
    MODEL_MAP = {
        ...
        {aimer::ModelType::STATUE, [](aimer::CoordConverter* const converter, aimer::EnemyState* const state) {
...
EnemyModelFactory::create_model(aimer::CoordConverter* const converter, aimer::EnemyState* const state) {
