...
void FilterThread::update(const aimer::ArmorData& armor, const double& t) {
  ...
  std::vector<double> r_vec{
      base::get_param<double>("auto-aim.armor-model.ekf.r.yaw"),
      base::get_param<double>("auto-aim.armor-model.ekf.r.pitch"),
      base::get_param<double>("auto-aim.armor-model.ekf.r.distance-at-1m") * ypd.dis * ypd.dis};
...
void ArmorModel::update() {
  ...
  for (const auto& d : armors) {
    ...
    if (it == this->filters.end()) {
      this->filters.insert(std::make_pair(d.id, FilterThread(this->converter, d, this->credit_time)));
...
aimer::AimInfo ArmorModel::get_positive_aim() const {
  ...
  for (const auto& d : this->filters) {
    if (!d.second.credit()
        || d.second.armor.info.area() < max_area * base::get_param<double>("auto-aim.armor-model.target-area-ratio")) {
      continue;
    ...
    aimer::AimInfo filter_aim = [&]() -> aimer::AimInfo {
      ...
      aimer::math::YpdCoord ypd = this->converter->aim_xyz_i_to_aim_ypd(shoot_param.aim_xyz_i_barrel);
  ...
  for (const auto& d : this->filters) {
    ...
    aimer::AimInfo filter_aim = [&]() -> aimer::AimInfo {
      ...
      return aimer::AimInfo(
          ...
          // 若仅考虑 yaw 就超误差，不跟随
          this->converter->aim_error_exceeded(
              aimer::math::YpdCoord(ypd.yaw, this->converter->get_control_aim_pitch0(), ypd.dis),
              this->state->get_sample_armor_ref(),
