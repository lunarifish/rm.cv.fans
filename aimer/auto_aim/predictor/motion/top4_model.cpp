...
bool RotateCounter::top_active() const {
  return !this->w_inactive()
         && std::fabs(this->get_active_rotate()) >= base::get_param<int64_t>("auto-aim.top-model.top4.active-rotate");
}
...
void RotateCounter::update_rotate(const std::vector<aimer::ArmorData>& armors) {
  ...
  if (armors.size() == 1 && this->pattern != top4::TopPattern::A && this->pattern != top4::TopPattern::B) {
  ...
  } else if (armors.size() == 2 && this->pattern != top4::TopPattern::AB && this->pattern != top4::TopPattern::BA) {
...
void RotateCounter::update_ab(const Eigen::Vector3d& center, const std::vector<top4::AbArmor>& ab, const double& t) {
  ...
  if (this->pattern_fixer.get_x_k1()(0, 0) > 0.) {
    this->pattern = this->pattern == top4::TopPattern::AB ? top4::TopPattern::BA : top4::TopPattern::AB;
...
TopModel::TopModel(aimer::CoordConverter* const converter, aimer::EnemyState* const state, const double& credit_time)
    : converter(converter),
      state(state),
...
std::vector<TopArmor> TopModel::predict_armors(const double& t) const {
  ...
  int index =
      (this->counter.get_pattern() == top4::TopPattern::A || this->counter.get_pattern() == top4::TopPattern::AB) ? 0
                                                                                                                  : 1;
...
aimer::AimInfo TopModel::get_limit_aim() const {
  ...
  std::vector<TopArmor> hit_results = this->predict_armors(this->converter->filter_to_hit_time(this->center_filter));
...
void TopModel::update() {  // 1 or 2
  ...
  if (this->counter.get_pattern() == top4::TopPattern::A) {
    aimer::debug::flask_aim
        << fmt::format("len in {:.3f}|#{:.3f}", this->counter.get_ab(0).length, this->counter.get_ab(1).length);
  } else if (this->counter.get_pattern() == top4::TopPattern::AB) {
    aimer::debug::flask_aim
        << fmt::format("len in {:.3f}|{:.3f}", this->counter.get_ab(0).length, this->counter.get_ab(1).length);
  } else if (this->counter.get_pattern() == top4::TopPattern::B) {
    aimer::debug::flask_aim
        << fmt::format("len in {:.3f}|#{:.3f}", this->counter.get_ab(1).length, this->counter.get_ab(0).length);
...
auto TopModel::length_sampling(
  ...
  Eigen::Vector3d center{center2(0, 0), center2(1, 0), (armors[0].info.pos(2, 0) + armors[1].info.pos(2, 0)) / 2.};
  double len_l = (l_pos2 - center2).norm();
  double len_r = (r_pos2 - center2).norm();

  // 绘制拟合 8 点
  {
    std::vector<std::vector<cv::Point2f>> double_pts = top::radial_double_pts(armors, z_to_l_fit, converter);
    // 绘制半径采样信息
...
double TopModel::double_angle_sampling(
  ...
  double z_to_l_fit_by_l = top::fit_single_z_to_v(armors[0], z_to_l_exp, M_PI / 2., M_PI, converter);
  double z_to_l_fit_by_r = [&]() {
    double z_to_r_fit = top::fit_single_z_to_v(armors[1], z_to_l_exp + M_PI / 2, M_PI, M_PI / 2. * 3., converter);
    return aimer::math::reduced_angle(z_to_r_fit - M_PI / 2.);
  }();
  // 若直接加权，需保证符号为正
  double z_to_l_fit_weight =
      aimer::math::get_weighted_angle(z_to_l_fit_by_l, armors[0].info.area(), z_to_l_fit_by_r, armors[1].info.area());
...
void TopModel::update_by_double_armors() {
  ...
  if (aimer::math::get_ratio(armors[0].info.area(), armors[1].info.area())
    ...
    TopModel::LengthSample length_sample = this->length_sampling(
        ...
        aimer::math::reduced_angle(
            this->counter.predict_theta_l(this->converter->get_img_t()) - this->converter->get_camera_z_i_yaw()
        ),
        this->converter
    );  // 包括角度取样
    ...
    aimer::debug::flask_aim
        << fmt::format("len sampLR {:.3f}|{:.3f}", length_sample.ab[0].length, length_sample.ab[1].length);
    ...
    this->counter.update_ab(length_sample.center, length_sample.ab, this->converter->get_img_t());
  } else {
  ...
  Eigen::Vector3d center_pro =
      (z_to_l_fix > M_PI / 4. * 3.)
          ? ([&]() {  // 懒得传参数
              Eigen::Vector2d l_norm2 = aimer::math::rotate(this->converter->get_camera_z_i2(), z_to_l_fix);
              // fix !
          ...
          : ([&]() {
              Eigen::Vector2d r_norm2 = aimer::math::rotate(this->converter->get_camera_z_i2(), z_to_l_fix + M_PI / 2.);
...
void TopModel::update_by_single_armor() {
  ...
  double z_to_l_fit = top::fit_single_z_to_v(armor, z_to_l_exp, M_PI / 4 * 3, M_PI / 4 * 5, this->converter);
...
void TopModel::draw_aim(cv::Mat& img) const {
  // 获取此刻结果
  std::vector<TopArmor> res = this->predict_armors(this->converter->get_img_t());  // 包含 Pattern 的具体实现
  // std::vector<TopArmor> res =
