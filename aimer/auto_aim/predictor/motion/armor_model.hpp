...
struct FilterThread {
 public:
  // 建立也是更新
  FilterThread(aimer::CoordConverter* const converter, const aimer::ArmorData& armor, const double& credit_time);
...
class ArmorModel {
 public:
  //  最不可缺少参数的放在参数列表前面
  ArmorModel(aimer::CoordConverter* const converter, aimer::EnemyState* const state, const double& credit_time);
  ~ArmorModel() = default;
  void update();
