...
double
get_pts_cost(const std::vector<cv::point2f>& cv_refs, const std::vector<cv::point2f>& cv_pts, const double& inclined) {
...
std::vector<Eigen::Vector3d> radial_armor_corners(
  ...
  const std::vector<cv::Point3d>& pw = type == aimer::ArmorType::BIG ? aimer::PW_BIG : aimer::PW_SMALL;
...
std::vector<cv::Point2f> radial_armor_pts(
  ...
  std::vector<Eigen::Vector3d> corners = top::radial_armor_corners(pos, type, pitch, z_to_v, converter);
...
double DoubleCost::operator()(const double& x) {
  ...
  std::vector<std::vector<cv::Point2f>> ml_pts = top::radial_double_pts(this->data.armors, x, this->data.converter);
  return top::get_pts_cost(
             ml_pts[0],
             std::vector<cv::Point2f>{
...
double fit_single_z_to_v(
  ...
  std::pair<double, double> res = solver.find(z_to_v_min, z_to_v_max, single_cost, top::FIND_ANGLE_ITERATIONS);
...
double fit_double_z_to_l(
  ...
  aimer::math::Trisection solver;
  std::pair<double, double> res = solver.find(z_to_l_min, z_to_l_max, double_cost, top::FIND_ANGLE_ITERATIONS);
  return aimer::math::reduced_angle(res.first);
}
...
aimer::AimInfo get_top_limit_aim(
  ...
  auto get_directs = [](const std::vector<top::TopArmor>& results) -> std::vector<top::TopArmor> {
    ...
    for (const auto& res : results) {
      if (std::fabs(res.zn_to_v)
          < aimer::math::deg_to_rad(base::get_param<double>("auto-aim.top-model.aim.max-orientation-angle"))) {
  ...
  auto choose_direct_aim =
      [&converter, &center_filter, &top_w, &get_direction](const std::vector<top::TopArmor>& directs) -> AimArmor {
    ...
    aimer::AimInfo aim = [&]() {
      ...
      aimer::math::YpdCoord ypd = converter->aim_xyz_i_to_aim_ypd(shoot_param.aim_xyz_i_barrel);
      // 计算 yaw_v
      ...
      // xy 方向上的速度
      Eigen::Vector2d xy_v = aimer::math::rotate(
                                 v_norm2,
      ...
      aimer::math::YpdCoord ypd_v =
          converter->filter_to_aim_ypd_v(center_filter) + converter->get_camera_ypd_v(direct.pos, xyz_v);
  ...
  auto get_indirect_aim = [&converter, &state, &center_filter, &top_w, &get_direction](
    ...
    for (const auto& armor : results) {
      ...
      double leaving_angle =
          (state->get_sample_armor_ref().width / 2. * base::get_param<double>("auto-aim.top-model.aim.max-out-error"))
          / armor.length;
      double armor_to_lim =
          aimer::math::reduced_angle(
              (get_direction(top_w) == TopDirection::CCW ? zn_to_lim - armor.zn_to_v : armor.zn_to_v - zn_to_lim) - M_PI
              + leaving_angle
          )
    ...
    // lim 度（例如 0）时的中心（欲瞄准
    // 用这个中心延长的位置）
    Eigen::Vector3d center_lim = center_filter.predict_pos(results[0].t + closest_to_lim / std::fabs(top_w));
    // what if top_w == 0?
    ...
    Eigen::Vector2d lim_norm2 = aimer::math::rotate(converter->get_camera_z_i2(), M_PI + zn_to_lim);
    Eigen::Vector2d emerging_pos2 = center_lim2 + lim_norm2 * indirect.length;
    Eigen::Vector3d emerging_pos = {emerging_pos2(0, 0), emerging_pos2(1, 0), center_lim(2, 0) + indirect.z_plus};
    aimer::AimInfo aim = [&]() {
  ...
  auto get_aim_angle = [&get_directs, &choose_direct_aim, &get_indirect_aim](const std::vector<top::TopArmor>& results
                       ) -> AimArmor {
...
void top_draw_aim(
  ...
  for (int i = 0; i < size; ++i) {
    ...
    aimer::debug::draw_line(img, pts_vec[i][3], pts_vec[p][0], color_sp * ((coe_i + coe_p) / 2.), thickness);
    aimer::debug::draw_line(img, pts_vec[i][2], pts_vec[p][1], color_sp * ((coe_i + coe_p) / 2.), thickness);
...
Eigen::Vector3d prolonged_center(
  ...
  center_pro << armor_pos(0, 0) - radius * radius_norm(0, 0), armor_pos(1, 0) - radius * radius_norm(1, 0),
      armor_pos(2, 0) - z_plus;
  return center_pro;
}
...
bool SimpleRotateCounter::w_active() const {  // 旋转状态
  return std::fabs(this->get_w()) > base::get_param<double>("auto-aim.top-model.active-w") / 180. * M_PI;
}

bool SimpleRotateCounter::w_inactive() const {
  return std::fabs(this->get_w()) <= base::get_param<double>("auto-aim.top-model.inactive-w") / 180. * M_PI;
}
...
void SimpleRotateCounter::update_rotate(const aimer::ArmorData& d, const double& zn_to_v, const double& t) {
  // 强修复
  // 当前帧区间
...
double OrientationSignFixer::fixed(const double& angle, const double& yaw) const {
  ...
  double credit_mid_yaw = aimer::math::get_weighted_angle(this->credit_min_yaw, 0.5, this->credit_max_yaw, 0.5);
...
void OrientationSignFixer::update(const double& yaw, const double& t) {
  ...
  if (this->update_t - this->init_t > this->sampling_time) {
    // 本帧暂时不知道是不是拓展
    // 自身可信，回头即重置
    double early_to_late = aimer::math::get_rotate_angle(this->reserver.get_early(), this->reserver.get_late());
    double mid = aimer::math::get_weighted_angle(this->reserver.get_early(), 0.5, this->reserver.get_late(), 0.5);
    double mid_to_cur = aimer::math::get_rotate_angle(mid, yaw);
    if (mid_to_cur * early_to_late < 0.) {
      // 新周期，重置存储器
      // wtf!!!!
      this->credit_max_yaw = aimer::math::max_angle(this->reserver.get_early(), this->reserver.get_late());
      this->credit_min_yaw = aimer::math::min_angle(this->reserver.get_early(), this->reserver.get_late());
      this->reserver.init(yaw);
      this->init_t = t;
    }
  }
...
void SimpleTopModel::update(const bool& enable_sign_fixer) {
  ...
  double zn_to_v_raw = [&]() {
    double zn_to_v = aimer::math::reduced_angle(
        top::fit_single_z_to_v(tracking_armor, M_PI / 4., M_PI / 2., M_PI / 2. * 3., this->converter) - M_PI
    );
    // 有时正负颠倒，但不影响 fabs 判断
    if (enable_sign_fixer) {
      return this->orientation_sign_fixer.fixed(zn_to_v, aimer::math::xyz_to_ypd(tracking_armor.info.pos).yaw);
    }
    return zn_to_v;
  }();
  ...
  // 获取和更新角度
  {
    double z_to_v_exp = aimer::math::reduced_angle(
        this->counter.predict_theta_v(this->converter->get_img_t()) - this->converter->get_camera_z_i_yaw()
    );  // reserve for debug
    double z_to_v_fit = aimer::math::reduced_angle([&]() {
      double zn_to_v = aimer::math::reduced_angle(
          top::fit_single_z_to_v(tracking_armor, z_to_v_exp, M_PI / 2., M_PI / 2. * 3., this->converter) - M_PI
      );
      if (enable_sign_fixer) {
        return this->orientation_sign_fixer.fixed(zn_to_v, aimer::math::xyz_to_ypd(tracking_armor.info.pos).yaw);
      }
  ...
  // 计算和更新中心
  {
    ...
    Eigen::Vector2d l_norm2 = aimer::math::rotate(this->converter->get_camera_z_i2(), z_to_v_fix);
    Eigen::Vector3d center_pro =
        top::prolonged_center(tracking_armor.info.pos, this->state->get_default_radius(), l_norm2, 0.);

    if (this->state->get_enemy_type() == aimer::EnemyType::OUTPOST) {
      const double center_update_max_angle =
          aimer::math::deg_to_rad(base::get_param<double>("auto-aim.top-model.outpost.center-update-max-angle"));
      ...
      if (!credit_before_update) {
        ...
        this->center_filter.update(
            ...
            this->converter->get_img_t(),
            {0.01, 10.},
            {base::get_param<double>("auto-aim.top-model.center-r-at-1m") * center_pro.norm() * center_pro.norm()}
        );
...
std::vector<top::TopArmor> SimpleTopModel::predict_armors(const double& t) const {
  ...
  for (int i = 0; i < this->armor_cnt; ++i) {
    ...
    Eigen::Vector3d pos{center(0, 0) + radius * norm2(0, 0), center(1, 0) + radius * norm2(1, 0), center(2, 0)};
