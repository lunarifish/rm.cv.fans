...
class TopModel {
 public:
  TopModel(aimer::CoordConverter* const converter, aimer::EnemyState* const state, const double& credit_time);
  // 少了少了，常数又少啦
  ~TopModel() = default;
 ...
 private:
  ...
  // 签名是私有的
  struct LengthSample {
    LengthSample(const Eigen::Vector3d& center, const double& z_to_l_fit, const std::vector<top4::AbArmor>& ab)
        : center(center),
  ...
  LengthSample length_sampling(const std::vector<aimer::ArmorData>&, const double&, aimer::CoordConverter*) const;
  double double_angle_sampling(const std::vector<aimer::ArmorData>&, const double&, aimer::CoordConverter*) const;
