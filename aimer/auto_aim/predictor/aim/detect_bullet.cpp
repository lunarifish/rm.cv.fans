...
bool test_is_bullet_color(const cv::Vec3b& hsv_col) {
  return hsv_col[2] > 50 && fabs((int)hsv_col[0] - 50) < 10 + .5 * exp((hsv_col[1] + hsv_col[2]) / 100);
}

// 做帧差（并与原来的取交）
cv::Mat
DoFrameDifference::get_diff(const cv::Mat& s1, const cv::Mat& s2, const cv::Mat& ref, const cv::Mat& lst_fr_bullets) {
    ...
    for (size_t x = 0; x < s1.cols; x += DIFF_STEP) {
      ...
      for (int dy = -0; dy < 1 && flag; ++dy) {
        ...
        for (int dx = -0; dx < 1 && flag; ++dx) {
          ...
          uint8_t tmp =
              (WEIGHTS[0] * abs(c1[0] - c2[0]) + WEIGHTS[1] * abs(c1[1] - c2[1]) + WEIGHTS[2] * abs(c1[2] - c2[2]))
...
void DetectBullet::get_bullets() {
  ...
  for (uint32_t i = 0; i < this->contours.size(); ++i) {
    ...
    if (this->test_is_bullet(contour)) {
      this->bullets.emplace_back(rect.center, std::min(rect_size.height, rect_size.width) * .5);
