...
auto ProjectileSimulator::get_pos_by_t(const double& t) const -> aim::HitPos {
  ...
  double h = (k * shoot_param.v0 * std::sin(shoot_param.aim_angle) + this->g) * k * w
             ...
             + this->g * std::log(1. - (k * w) / (shoot_param.v0 * std::cos(shoot_param.aim_angle))) / k / k;
  // 弹道轨迹仅取决于目标点
  ...
  const Eigen::Vector3d bullet_xyz_i_camera = this->converter->xyz_i_barrel_to_xyz_i_camera(bullet_xyz_i_barrel);
  const Eigen::Vector2d bullet_xy_i_barrel = {bullet_xyz_i_barrel(0, 0), bullet_xyz_i_barrel(1, 0)};
  const Eigen::Vector2d target_xy_i_barrel = {target_xyz_i_barrel(0, 0), target_xyz_i_barrel(1, 0)};
  return aim::HitPos{bullet_xy_i_barrel.norm() >= target_xy_i_barrel.norm(), bullet_xyz_i_camera};
}
...
auto ProjectileSimulator::get_circle_by_t(const double& t) const -> aim::HitCircle {
  ...
  Eigen::Vector3d edge_xyz_c = xyz_c + crossed * base::get_param<double>("launching-mechanism.bullet.radius");
...
auto ProjectileSimulator::catch_circle(const aimer::math::CircleF& circle) const -> aim::CaughtCost {
  ...
  return aim::CaughtCost{
      dis <= aim::CATCH_CIRCLE_DIS_MAX_RATIO * hit_circle.circle.r && r_ratio >= aim::CATCH_CIRCLE_RADIUS_MIN_RATIO,
      dis / (aim::CATCH_CIRCLE_DIS_MAX_RATIO * hit_circle.circle.r)
          + (1. - r_ratio) / (1. - aim::CATCH_CIRCLE_RADIUS_MIN_RATIO)};
}

auto ProjectileSimulator::fit_circle(const aimer::math::CircleF& circle) const -> aimer::math::CircleF {
  // 写一个二分类还是手写二分
  ...
  double t =
      solver.find(this->fire_t, this->fire_t + aim::FIT_CIRCLE_MAX_T, cost_func, aim::FIT_CIRCLE_ITERATIONS_NUM).first;
...
AimCorrector::AimCorrector(aimer::CoordConverter* const converter)
    : converter{converter},
      bullet_detector(aim::DoReproj(this->converter->get_f_cv_mat_ref(), this->converter->get_rot_ic_sup_cv_mat_ref())),
...
auto AimCorrector::update_bullet_id(const int& last_shoot_id) -> void {
  ...
  while (true) {
    ...
    double fire_controlling_t = origin_aim.img_t + this->converter->get_control_to_fire_latency();
    // + this->converter->get_img_to_fire_latency() -
    //     this->converter->get_img_to_control_latency();
    // 寻找 fire_controlling_t 后的第一个 aim 记录
    aim::IdTLatencyAimCorrection fire_controlling_aim = this->aim_history.find_by_img_t(fire_controlling_t);
    ...
    if (this->bullets.size() + 1u <= aim::AIM_CORRECTOR_BULLETS_MAX_SZ) {
      ...
      this->bullets.push_back(aim::IdProj{
          ...
          aim::ProjectileSimulator(
              ...
              origin_aim.img_t + origin_aim.img_to_predict_latency + this->converter->get_predict_to_send_latency()
                  + this->converter->get_send_to_control_latency() + this->converter->get_control_to_fire_latency()
          )});
    }
    this->pending_ids.pop();
    continue;
...
auto AimCorrector::sample_aim_errors() -> void {
  ...
  this->bullet_detector.process_new_frame(this->converter->get_img_ref(), this->converter->get_q());
  ...
  for (const auto& bullet : this->bullets) {
    ...
    if (best_bullet != undistorted_detected.end()) {
      ...
      // 实际 - 预期（理想发射）
      Eigen::Vector2d yp_error = {caught_yp.yaw - fit_yp.yaw, caught_yp.pitch - fit_yp.pitch};
      {
        std::vector<double> r_vec = {base::get_param<double>("auto-aim.aim-corrector.error.r")};
      ...
      if (this->error_angles.size() + 1u > aim::AIM_CORRECTOR_ERROR_ANGLES_MAX_SZ && !this->error_angles.empty()) {
  ...
  }
  {
    ...
    for (auto& d : this->error_angles) {
      i += 1;
      aimer::debug::auto_aim_page()->sub("aim_corrector_采样数据").sub("error" + std::to_string(i)).get() =
          fmt::format("{:.2f}, {:.2f}", aimer::math::rad_to_deg(d(0, 0)), aimer::math::rad_to_deg(d(1, 0)));
    }
  }
}

auto AimCorrector::get_aim_error() const -> Eigen::Vector2d {
  return Eigen::Vector2d(this->error_filters[0].predict(0.)(0, 0), this->error_filters[1].predict(0.)(0, 0));
}
