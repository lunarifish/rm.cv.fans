...
void pnp_get_pc(
  ...
  std::vector<cv::Point2d> pts{detected.pts[0], detected.pts[1], detected.pts[2], detected.pts[3]};
  cv::Mat t_vec, r_vec;
...
aimer::ArmorInfo detected_to_info(
  ...
  const std::array<cv::Point2f, 4> pts = {detected.pts[0], detected.pts[1], detected.pts[2], detected.pts[3]};
  ...
  const aimer::PnpDistanceFixer pnp_distance_fixer(0., 1., base::get_param<double>("auto-aim.pnp.distance-fixer.a2"));
  const aimer::math::YpdCoord ypd_c_feature_fixed{ypd_c.yaw, ypd_c.pitch, pnp_distance_fixer.fixed_dis(ypd_c.dis)};
