...
LightThread::LightThread(const int& id, const aimer::ArmorInfo& armor_info, const double& t, const int& frame)
    : id(id),
...
bool LightThread::alive(const ::ArmorColor& enemy_color, const double& t) const {
  return this->target_color == static_cast<int>(enemy_color) && t - this->close_target_t <= aimer::LIGHT_LIFE;
}
...
double LightThread::get_cost(const aimer::ArmorInfo& armor_info) const {
  ...
  // 消除 dis 过大误差的影响
  Eigen::Vector3d out_pos = aimer::math::ypd_to_xyz(aimer::math::YpdCoord(out_ypd.yaw, out_ypd.pitch, in_ypd.dis));
  // 对角线长度的一半为 0.5
  cost += (in_pos - out_pos).norm()
          / (aimer::math::get_norm(this->armor_info.sample.width, this->armor_info.sample.height) / 2.) * 0.5;
  return cost;
}
...
void LightManager::update(const std::vector<aimer::ArmorInfo>& armor_infos) {
  ...
  for (auto& d : info_list) {  // 用来更新的 armors 已经删了
    if (d.detected.color == static_cast<int>(this->converter->get_robot_status_ref().enemy_color)) {
      ++this->light_id_cnt;
