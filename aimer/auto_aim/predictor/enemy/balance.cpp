...
void BalanceJudger::update(const std::vector<aimer::ArmorData>& armors, const double& t, const int& frame) {
  // size: 能看到装甲板的数量
  if (armors.empty()) {
    return;
  }
  ...
  if (frame == this->last_frame + 1 && credit_size == this->last_size) {
    if (credit_size >= 2) {
      this->double_conf += 0.5 * (t - this->last_t)
                           / base::get_param<double>("auto-aim.balance-judger.non-balance-confidence-ramp-time");
    } else {
      this->double_conf -=
          0.5 * (t - this->last_t) / base::get_param<double>("auto-aim.balance-judger.balance-confidence-ramp-time");
    }
    // if (double_cons == 0)
    //   double_conf -= 0.00033;
    // else
