...
class EnemyState {
 ...
 private:
  ...
  screened_armors(const std::vector<aimer::ArmorData>& raw_armors, const std::vector<aimer::ArmorData>& last_armors);

  // 根据指定 size 裁剪和排序装甲板
  std::vector<aimer::ArmorData> sorted_armors(const std::vector<aimer::ArmorData>& src, const int& max_size);
};
}  // namespace aimer
