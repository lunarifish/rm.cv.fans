...
const std::unordered_map<aimer::EnemyType, aimer::EnemyInfo> ENEMY_INFO{
    ...
    {aimer::EnemyType::HERO,
     aimer::EnemyInfo{aimer::BIG_ARMOR, aimer::math::deg_to_rad(15.), 4, 2, 0.25, INFANTRY_AIM_MAX_ERROR_STR}},
    {aimer::EnemyType::ENGINEER,
     aimer::EnemyInfo{aimer::SMALL_ARMOR, aimer::math::deg_to_rad(15.), 4, 2, 0.25, INFANTRY_AIM_MAX_ERROR_STR}},
    {aimer::EnemyType::INFANTRY,
     aimer::EnemyInfo{aimer::SMALL_ARMOR, aimer::math::deg_to_rad(15.), 4, 2, 0.25, INFANTRY_AIM_MAX_ERROR_STR}},
    {aimer::EnemyType::BALANCE_INFANTRY,
     EnemyInfo{aimer::BIG_ARMOR, aimer::math::deg_to_rad(15.), 2, 1, 0.473 / 2., INFANTRY_AIM_MAX_ERROR_STR}},
    {aimer::EnemyType::OUTPOST,
     EnemyInfo{aimer::SMALL_ARMOR, aimer::math::deg_to_rad(-15.), 3, 2, 0.553 / 2., INFANTRY_AIM_MAX_ERROR_STR}},
    {aimer::EnemyType::CRYSTAL_BIG,
     EnemyInfo{aimer::BIG_ARMOR, aimer::math::deg_to_rad(15.), 1, 1, 0.25, INFANTRY_AIM_MAX_ERROR_STR}},
    {aimer::EnemyType::CRYSTAL_SMALL,
     EnemyInfo{aimer::SMALL_ARMOR, aimer::math::deg_to_rad(15.), 1, 1, 0.25, INFANTRY_AIM_MAX_ERROR_STR}}};
...
EnemyState::EnemyState(const int& number, const aimer::EnemyType& init_type) : number(number), enemy_type(init_type) {}

void EnemyState::update(const std::vector<aimer::ArmorData>& raw_armors, const double& t, const int& frame) {
  // 排除不可能的装甲板（太远，太小）
  ...
  if (this->number >= 3 && this->number <= 5) {
    ...
    if (!base::get_param<bool>("auto-aim.enemy-state.manually-set-balance-infantry")) {
      aimer::EnemyType enemy_type_to_be =
          this->balance_judger.is_balance() ? aimer::EnemyType::BALANCE_INFANTRY : aimer::EnemyType::INFANTRY;
      this->switch_enemy_type(enemy_type_to_be);
    } else {
      // 根据参数切换敌人类型
      std::string param_str =
          std::string("auto-aim.enemy-state.infantry") + std::to_string(this->number) + std::string("-is-balance");
      aimer::EnemyType enemy_type_to_be =
          base::get_param<bool>(param_str) ? aimer::EnemyType::BALANCE_INFANTRY : aimer::EnemyType::INFANTRY;
      this->switch_enemy_type(enemy_type_to_be);
    }
  }
  ...
  for (auto it = armors.begin(); it != armors.end();) {
    ...
    if (found_id) {
      ...
      if (it->info.area() < max_area * aimer::NEW_ARMOR_CRED_AREA_RATIO
          || it->info.pos.norm() > base::get_param<double>("auto-aim.enemy-state.new-armor-max-valid-distance")) {
        it = armors.erase(it);
        continue;
      }
    }
...
// 根据指定 size 裁剪和排序装甲板
std::vector<aimer::ArmorData> EnemyState::sorted_armors(const std::vector<aimer::ArmorData>& src, const int& max_size) {
