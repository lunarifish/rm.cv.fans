...
std::vector<ArmorData> OutpostFixer::filter(const std::vector<ArmorData>& armors, const double& request_t) {
  const double z_that_distinguishes = base::get_param<double>("auto-aim.outpost-fixer.z-that-distinguishes-two-types");
  const double active_lasting_time = base::get_param<double>("auto-aim.outpost-fixer.active-lasting-time");
