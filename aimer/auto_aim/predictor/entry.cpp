...
void predictor_run() {
  ...
  // 第一视角自瞄信息
  auto predictor_aim_checkbox = umt::ObjManager<::CheckBox>::find_or_create("auto_aim.predictor.aim");
  auto webview_predictor_aim = umt::Publisher<cv::Mat>("auto_aim.predictor.aim");

  // 第三视角自瞄信息
  auto predictor_map_checkbox = umt::ObjManager<::CheckBox>::find_or_create("auto_aim.predictor.map");
  ...
  while (true) {
    try {
      ...
      if (std::isnan(send.yaw) || std::isnan(send.pitch) || std::isinf(send.yaw) || std::isinf(send.pitch)) {
        fmt::print(
