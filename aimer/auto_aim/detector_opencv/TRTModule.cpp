  ...
  try {
    ...
    for (int i = 0; i < y.size[1]; i++) {
      ...
      if (result[8] < inv_sigmoid(KEEP_THRES))
        continue;
      if (result[8]
          < inv_sigmoid(this->keep_thres[argmax(result + 2 * this->points_num + 1 + this->colors_num, this->tags_num)]))
        continue;
      bbox_t box;
      for (int i = 0; i < 4; i++) {
      ...
      box.tag_id = argmax(result + 2 * this->points_num + 1 + this->colors_num, this->tags_num);
      box.confidence = sigmoid(result[2 * this->points_num]);
      box.confidence_cls = sigmoid(result[2 * this->points_num + 1 + this->colors_num + box.tag_id]);
