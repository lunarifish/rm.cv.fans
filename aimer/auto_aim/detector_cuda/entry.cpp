...
void draw_pts(const cv::Mat& src, cv::Mat& dst, const std::vector<bbox_t>& boxs) {
  ...
  static const std::string names[36] = {"BG", "B1",  "B2",  "B3", "B4",  "B5",  "BO", "BBb", "BBs", "RG", "R1",  "R2",
                                        "R3", "R4",  "R5",  "RO", "RBb", "RBs", "NG", "N1",  "N2",  "N3", "N4",  "N5",
                                        "NO", "NBb", "NBs", "PG", "P1",  "P2",  "P3", "P4",  "P5",  "PO", "PBb", "PBs"};
  ...
  for (auto box : boxs) {
    ...
    cv::putText(dst, names[box.color_id * 9 + box.tag_id], box.pts[0], cv::FONT_HERSHEY_SIMPLEX, 1, colors[1], 1);
    cv::putText(dst, std::to_string(box.confidence), box.pts[2], cv::FONT_HERSHEY_SIMPLEX, 1, colors[1], 1);
  }
}
...
bool get_Dist(bbox_t res1, bbox_t res2) {
  ...
  thresh = 0.5 * sqrt(pow((res1.pts[0].x - res2.pts[3].x), 2) + pow((res1.pts[0].y - res2.pts[3].y), 2));
