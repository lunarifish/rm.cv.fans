...
struct ArmorInfo {
  ...
  bool valid() const {
    ...
    for (int i = 0; i < 4; ++i) {
      if (aimer::math::is_nan_or_inf(this->detected.pts[i].x) || aimer::math::is_nan_or_inf(this->detected.pts[i].y)) {
        return false;
      }
    }
    for (int i = 0; i < 3; ++i) {
  ...
  cv::Point2f center() const {
    return cv::Point2f{
        (this->detected.pts[0].x + this->detected.pts[1].x + this->detected.pts[2].x + this->detected.pts[3].x) / 4.f,
        (this->detected.pts[0].y + this->detected.pts[1].y + this->detected.pts[2].y + this->detected.pts[3].y) / 4.f};
  }
};

struct ArmorData {
  // 生成一个装甲板实例数据
  ArmorData(const int& id, const int& color, const aimer::ArmorInfo& info) : id(id), color(color), info(info) {}
  bool is_hit() const {
