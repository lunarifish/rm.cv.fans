...
pos_to_map_point(const Eigen::Vector3d& pos, const cv::Scalar& color, const int& radius, const int& thickness) {
  return FlaskPoint(
      {float(FLASK_MAP_MID_X + pos(0, 0) * base::get_param<double>("auto-aim.debug.flask.map.pixel-per-meter")),
       float(FLASK_MAP_MID_Y - pos(1, 0) * base::get_param<double>("auto-aim.debug.flask.map.pixel-per-meter"))},
...
std::vector<FlaskLine> poses_to_map_lines(
  ...
  for (std::size_t i = 0; i < poses.size(); ++i) {
    ...
    lines.emplace_back(std::pair<cv::Point2f, cv::Point2f>(pt1.pt, pt2.pt), (pt1.color + pt2.color) / 2., thickness);
  }
  return lines;
}
...
auto Stm32Shoot::get_last_shoot_id(const double& img_t) -> int {
  // 实际上是传输过去有延迟，
  while (!this->pending_signals.empty() && img_t >= this->pending_signals.front().img_t + Stm32Shoot::SHOOT_LATENCY) {
    // 信号已经到达，进行信号处理
    if (this->pending_signals.front().img_t
        >= this->last_shoot.img_t + base::get_param<double>("auto-aim.ec-simulator.shoot-interval")) {
