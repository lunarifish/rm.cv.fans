...
class PeriodicRecorder {
 public:
  PeriodicRecorder(const std::function<bool(const ValueT&, const ValueT&)>& cmp, const PeriodT& init_stamp)
      : cmp(cmp),
        stamp(init_stamp) {}
...
class PeriodicAverage {
 public:
  PeriodicAverage(const std::size_t& buf_size, const PeriodT& init_stamp) : buf_size(buf_size), stamp(init_stamp) {}
...
struct FlaskPoint {
  FlaskPoint(const cv::Point2f& pt, const cv::Scalar& color, const int& radius, const int& thickness)
      : pt(pt),
        color(color),
        radius(radius),
        thickness(thickness) {}
...
struct FlaskLine {
  FlaskLine(const std::pair<cv::Point2f, cv::Point2f>& pt_pair, const cv::Scalar& color, const int& thickness)
      : pt_pair(pt_pair),
        color(color),
        thickness(thickness) {}
...
struct FlaskText {
  FlaskText(const std::string& str, const cv::Point2f& pt, const cv::Scalar& color, const double& scale)
      : str(str),
        pt(pt),
        color(color),
        scale(scale) {}
...
FlaskPoint
pos_to_map_point(const Eigen::Vector3d& pos, const cv::Scalar& color, const int& radius, const int& thickness);
...
FlaskText
pos_str_to_map_text(const std::string& str, const Eigen::Vector3d& pos, const cv::Scalar& color, const double& scale);
 ...
 public:
  ...
  FlaskStream& operator>>(cv::Mat& img) {
    ...
    for (auto& str : this->logs) {
      cv::putText(img, str, {20, 80 + cnt * 24}, cv::FONT_HERSHEY_DUPLEX, 0.8, {0, 0, 255});
      ++cnt;
    }
    ...
    for (auto& text : this->texts) {
      cv::putText(img, text.str, {int(text.pt.x), int(text.pt.y)}, cv::FONT_HERSHEY_DUPLEX, text.scale, text.color);
    }
    return *this;
  }
...
inline auto start_end_time_to_fmt_pair(const debug::StartEndTime& x) -> std::pair<std::string, std::string> {
