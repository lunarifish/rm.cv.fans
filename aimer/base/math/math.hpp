...
inline float get_area(const cv::Point2f* pts) {
  return std::fabs(math::get_cross(pts[0], pts[1], pts[2])) + std::fabs(math::get_cross(pts[0], pts[2], pts[3]));
}
...
inline double get_weighted_angle(const double& theta1, const double& w1, const double& theta2, const double& w2) {
...
inline Eigen::Vector2d get_intersection(
  ...
  return math::get_intersection(math::pt_and_norm_to_line(pt_a, norm_a), math::pt_and_norm_to_line(pt_b, norm_b));
}
...
class Bisection {
 public:
  ...
  std::pair<ValueT, ValueT> find(ValueT left, ValueT right, Func&& cost_function, const int& iterations_num) {
