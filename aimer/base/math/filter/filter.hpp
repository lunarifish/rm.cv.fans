...
class SingleFilter {
 public:
  ...
  void update(const double& x, const double& t, const std::vector<double>& q_vec, const std::vector<double>& r_vec) {
...
class PositionFilter : public PositionPredictorInterface {
 public:
  ...
  Eigen::Vector3d predict_pos(const double& t) const override {  // const 这个是不得不加
    Eigen::Vector3d res;
    res << this->kalman[0].predict(t)(0, 0), this->kalman[1].predict(t)(0, 0), this->kalman[2].predict(t)(0, 0);
    return res;
  }
  ...
  Eigen::Vector3d predict_v(const double& t) const override {
    ...
    Eigen::Vector3d res;
    res << this->kalman[0].predict(t)(1, 0), this->kalman[1].predict(t)(1, 0), this->kalman[2].predict(t)(1, 0);
    return res;
  }
...
class AngleFilter {
 public:
  ...
  void update(const double& angle, const double& t) {
    this->filter.update(aimer::math::get_closest(angle, this->filter.predict(t)(0, 0), this->range), t);
    // 强制修正内部更新的角度
  ...
  void update(const double& angle, const double& t, const double& R) {
    this->filter.update(aimer::math::get_closest(angle, this->filter.predict(t)(0, 0), this->range), t, R);
    this->filter.set_x(aimer::math::reduced(this->filter.get_x_k1()(0, 0), this->range));
  }
