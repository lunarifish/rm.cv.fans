...
struct AimInfo {
  ...
  static const AimInfo idle() {
    return AimInfo(aimer::math::YpdCoord(), aimer::math::YpdCoord(), aimer::ShootParam(), ::ShootMode::IDLE);
  }
...
class PnpDistanceFixer {
 public:
  PnpDistanceFixer(const double& a0, const double& a1, const double& a2) : a0(a0), a1(a1), a2(a2) {}
...
class CoordConverter {
 private:
  ...
  std::shared_ptr<::RobotStatus> robot_status_obj;  // 由 /core_io 中 Serial port （串行接口） 读入并
 ...
 public:
  ...
  // 此为利用相机的坐标系系统所求出的近似值
  aimer::math::YpdCoord get_camera_ypd_v(const Eigen::Vector3d& xyz_i, const Eigen::Vector3d& xyz_v_i) const;
  // pts 转标准化 pis
