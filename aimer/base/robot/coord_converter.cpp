...
class ResistanceFuncLinear {
 ...
 public:
  ...
  bool operator()(const T* const x, T* residual) const {
    ...
    residual[0] = (k * this->v0 * ceres::sin(x[0]) + this->g) * k * this->w / (k * k * this->v0 * ceres::cos(x[0]))
                  + this->g * ceres::log(1. - (k * this->w) / (this->v0 * ceres::cos(x[0]))) / k / k - this->h;
    return true;
  }
};
...
CoordConverter::CoordConverter() : robot_status_obj{umt::ObjManager<RobotStatus>::find_or_create("robot_status")} {
...
double CoordConverter::get_yaw_compensate() const {
  return base::get_param<bool>("ec.using-angle-compensate") ? this->get_robot_status_ref().yaw_compensate / 180. * M_PI
                                                            : 0.;
}
...
double CoordConverter::get_img_to_prediction_latency(const Eigen::Vector3d& aim_xyz_i_barrel) const {
  return this->get_img_to_predict_latency() + this->get_predict_to_send_latency() + this->get_send_to_control_latency()
...
double CoordConverter::get_img_to_hit_latency(const Eigen::Vector3d& aim_xyz_i_barrel) const {
  return this->get_img_to_predict_latency() + this->get_predict_to_send_latency() + this->get_send_to_control_latency()
         + this->get_control_to_fire_latency() + this->get_fire_to_hit_latency(aim_xyz_i_barrel);
}
...
void CoordConverter::catch_send_timestamp() {  // and latency
  double send_timestamp =
      std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count()
      / 1e3;  // 目前与陀螺仪的触发频率统一
  this->predict_to_send_latency_filter
      .update(send_timestamp - this->predict_timestamp_binder.get(this->get_frame()), this->get_img_t(), {1.}, {3000.});
}  // 统计估计
...
bool CoordConverter::aim_error_exceeded(
  ...
  if (this->aim_to_horiz_diff(ypd) > sample_armor.width / 2. * std::cos(yaw_inclined_camera) * error_rate) {
    return true;
  }
  if (this->aim_to_vert_diff(ypd)
      > sample_armor.height / 2. * std::cos(pitch_inclined_imu + this->get_camera_z_i_pitch()) * error_rate) {
...
bool CoordConverter::aim_error_exceeded(
  ...
  if (aimer::math::get_termination_dis(ideal_ypd.dis, real_ypd.pitch, ideal_ypd.pitch)
      > sample_armor.height / 2. * std::cos(pitch_inclined_world + this->get_camera_z_i_pitch()) * error_rate) {
    // 对方朝上 15 度，我朝下 15 度，这是刚好对准的
    return true;
  }
  return false;
...
// 此为利用相机的坐标系系统所求出的近似值
aimer::math::YpdCoord CoordConverter::get_camera_ypd_v(const Eigen::Vector3d& xyz_i, const Eigen::Vector3d& xyz_v_i)
    const {
  return aimer::math::camera_get_ypd_v(this->pi_to_pc(xyz_i), this->pi_to_pc(xyz_v_i));
}

// pts 转标准化 pis
std::vector<Eigen::Vector3d> CoordConverter::pts_to_pis_norm(const std::vector<cv::Point2f>& pts) const {
  std::vector<cv::Point2f> pus;
...
aimer::ShootParam CoordConverter::target_pos_to_shoot_param(const Eigen::Vector3d& target_pos) const {
  ...
  const double target_xy = std::sqrt(
      target_xyz_i_barrel(0, 0) * target_xyz_i_barrel(0, 0) + target_xyz_i_barrel(1, 0) * target_xyz_i_barrel(1, 0)
  );
  ...
  const double shoot_angle = [&target_xy, &target_z, &bs]() {
    ...
    problem.AddResidualBlock(
        new ceres::AutoDiffCostFunction<ResistanceFuncLinear, 1, 1>(new ResistanceFuncLinear(target_xy, target_z, bs)),
        nullptr,
        &shoot_angle
    );
...
double CoordConverter::filter_to_prediction_time(const PositionPredictorInterface& filter) const {
  ...
  for (int i = 0; i < iterations_num; ++i) {
    prediction_t =
        this->get_prediction_time(this->target_pos_to_shoot_param(filter.predict_pos(prediction_t)).aim_xyz_i_barrel);
  }
  return prediction_t;
}

aimer::ShootParam CoordConverter::filter_to_shoot_param(const PositionPredictorInterface& filter) const {
...
// aim 总是关于枪口的，这里存在误差
aimer::math::YpdCoord CoordConverter::filter_to_aim_ypd_v(const PositionPredictorInterface& filter) const {
...
double CoordConverter::filter_to_hit_time(const PositionPredictorInterface& filter) const {
  ...
  for (int i = 0; i < iterations_num; ++i) {
    hit_t = this->get_hit_time(this->target_pos_to_shoot_param(filter.predict_pos(hit_t)).aim_xyz_i_barrel);
  }
  return hit_t;
}
...
aimer::math::YpdCoord CoordConverter::filter_to_hit_aim_ypd(const PositionPredictorInterface& filter) const {
  return this->target_pos_to_aim_ypd(this->filter_to_hit_pos(filter));
}

// aim 总是关于枪口的，这里存在误差
aimer::math::YpdCoord CoordConverter::filter_to_hit_aim_ypd_v(const PositionPredictorInterface& filter) const {
...
void CoordConverter::update(const cv::Mat& img, const Eigen::Quaternionf& q, const double& timestamp) {
  this->frame += 1;
  this->img = img;
